...
int fdt_parse_aplic_node(void *fdt, int nodeoff, struct aplic_data *aplic)
{
	...
	val = fdt_getprop(fdt, nodeoff, "aplic,num-sources", &len);
	if (len > 0)
	...
	if (val && len >= sizeof(fdt32_t)) {
		...
		val = fdt_getprop(fdt, nodeoff, "aplic,children", &len);
		if (!val || len < sizeof(fdt32_t))
			goto aplic_msi_parent_done;
...
aplic_msi_parent_done:
	...
	del = fdt_getprop(fdt, nodeoff, "aplic,delegate", &len);
	...
	for (i = 0; i < dcnt; i += 3) {
		...
		deleg->first_irq = fdt32_to_cpu(del[i]);
		deleg->last_irq = fdt32_to_cpu(del[i + 1]);
		deleg->child_index = 0;
...
int fdt_parse_imsic_node(void *fdt, int nodeoff, struct imsic_data *imsic)
{
	...
	val = fdt_getprop(fdt, nodeoff, "imsic,guest-index-bits", &len);
	if (val && len > 0)
		imsic->guest_index_bits = fdt32_to_cpu(*val);
	else
		imsic->guest_index_bits = 0;

	val = fdt_getprop(fdt, nodeoff, "imsic,hart-index-bits", &len);
	if (val && len > 0) {
	...
	val = fdt_getprop(fdt, nodeoff, "imsic,group-index-bits", &len);
	if (val && len > 0)
		imsic->group_index_bits = fdt32_to_cpu(*val);
	else
		imsic->group_index_bits = 0;

	val = fdt_getprop(fdt, nodeoff, "imsic,group-index-shift", &len);
	if (val && len > 0)
		imsic->group_index_shift = fdt32_to_cpu(*val);
	else
		imsic->group_index_shift = 2 * IMSIC_MMIO_PAGE_SHIFT;

	val = fdt_getprop(fdt, nodeoff, "imsic,num-ids", &len);
	if (val && len > 0)
